...
allowed-tools: Bash, Read, Grep, Glob, mcp__blackbird__blackbird_reservation_release, mcp__blackbird__blackbird_message_send, mcp__blackbird__blackbird_message_reply, mcp__blackbird__blackbird_agents_list, mcp__blackbird__blackbird_conversation_open
---
...
2. **Send the durable handoff** with `blackbird_message_send` (or
   `blackbird_message_reply` if this continues an existing conversation). The
...
3. **Release every lease** with `blackbird_reservation_release`, passing the
   `lease_id` and current `fences`. Release rather than letting leases expire —
