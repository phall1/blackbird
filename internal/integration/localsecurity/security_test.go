...
func TestTranscriptSignatureAndPairingProofReplayBinding(t *testing.T) {
	...
	// The transport supplies a distinct channel binding per connection; this
	// boundary only consumes them, so two unequal values stand in for two
	// separate connections.
	firstBinding := Binding(sha256.Sum256([]byte("first connection binding")))
	secondBinding := Binding(sha256.Sum256([]byte("second connection binding")))
...
func TestSecurityPrimitivesRejectMissingInputs(t *testing.T) {
	...
	if _, err := PairingProof(make([]byte, 32), Binding{}, TranscriptHash{1}); !errors.Is(err, ErrInvalidProof) {
		t.Fatalf("zero channel binding error = %v", err)
	}
}
...
func TestVaultSignerCompositionAndNoSecretLeakEvidence(t *testing.T) {
	...
	logger.Printf(
		"credential=%v invitation=%v reference=%v error=%v",
		credential, invitation, reference, ErrAccessDenied,
	)
	...
	}{reference.String(), publicKey, invitationID.String(), invitation})
	if err != nil {
		t.Fatal(err)
	}
...
func TestVaultRecoveryCapsuleSignerLookupRejectsForgeryAndRedacts(t *testing.T) {
	...
	if adapters.DenialPolicy == nil || adapters.Authentication == nil ||
...
func TestPairingRedemptionVerifierConstructsBoundAuthorization(t *testing.T) {
	...
	assurance, _ := domain.NewAssuranceClass("hardware_key")
	// The daemon only compares the device SPKI fingerprint it was given; it
	// never derives one, so any stable digest stands in for the pairing client's.
	spkiDigest, _ := domain.NewCredentialDigest(sha256.Sum256(privateKey.Public().(ed25519.PublicKey)))
