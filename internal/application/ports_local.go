...
// CoordinationWaitReason names which of the three things a bounded wait was
...
type CoordinationWaitReason string

const (
	// CoordinationWaitPathFree means no lease conflicting with the requested
	// path and mode is active any more, so an acquisition is worth retrying.
	CoordinationWaitPathFree CoordinationWaitReason = "path_free"
	// CoordinationWaitMailArrived means a delivery addressed to the caller was
	// created after the wait began.
	CoordinationWaitMailArrived CoordinationWaitReason = "mail_arrived"
	// CoordinationWaitDeadline means neither happened before the budget ran
	...
	CoordinationWaitDeadline CoordinationWaitReason = "deadline"
)

const (
	// MaxCoordinationWait is the ceiling the store imposes on every wait,
	...
	MaxCoordinationWait = 60 * time.Second
	// CoordinationWaitPoll is how often a wait re-reads the condition. A wait
	...
	CoordinationWaitPoll = 250 * time.Millisecond
)

// CoordinationWaitRequest is a bounded server-side long poll. MCP gives a model
...
type CoordinationWaitRequest struct {
	...
	AwaitMail bool
	// Timeout is clamped to MaxCoordinationWait; zero asks for the ceiling.
	Timeout time.Duration
}

// CoordinationWaitResult reports what ended the wait and the evidence for it.
...
type CoordinationWaitResult struct {
	Reason CoordinationWaitReason
