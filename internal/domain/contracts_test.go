...
func TestProvisioningScopeIsSeparateAndPurposeBound(t *testing.T) {
	...
	invitation, _ := ParseInvitationID(validUUIDV7)
	authorityScope, _ := InstallationScope(installation)
	operation, _ := NewOperationName("installation.bootstrap")
	key, _ := NewIdempotencyKey("bootstrap-1")
	fingerprint := FingerprintCommand([]byte("signed transcript"))
	scope, err := NewProvisioningIdempotencyScope(authorityScope, invitation, fingerprint, operation, key)
	if err != nil || scope.InvitationID() != invitation || scope.TranscriptFingerprint() != fingerprint {
		t.Fatalf("scope = %#v, %v", scope, err)
	}
	if _, err := NewProvisioningIdempotencyScope(authorityScope, invitation, CommandFingerprint{}, operation, key); !errors.Is(err, ErrInvalidScope) {
...
func TestSessionBindingCapturesExactSecuritySnapshot(t *testing.T) {
	...
	binding, err := NewSessionBinding(
		...
		membershipRef, delegationRef, &deviceRef, grants,
	...
	if _, err := NewSessionBinding(
		authority, epoch, workspace, principal, actor,
		membershipRef, delegationRef, nil, []AggregateRef{grantRef, grantRef},
		policy, assurance, issuedAt, expiresAt,
	...
	if _, err := NewSessionBinding(
		authority, epoch, workspace, principal, actor,
		membershipRef, delegationRef, nil, nil,
