// Package mcp exposes local coordination through the official MCP Go SDK.
package mcp
...
// MetricsObserver receives bounded operational labels only.
...
func NewServer(dependencies Dependencies) (*Server, error) {
	if isNil(dependencies.Coordination) {
		return nil, errors.New("mcp transport requires a local coordination store")
	}
	sdk := sdkmcp.NewServer(&sdkmcp.Implementation{Name: "blackbird", Version: "v1"}, &sdkmcp.ServerOptions{
		Capabilities: &sdkmcp.ServerCapabilities{}, PageSize: 64,
	})
