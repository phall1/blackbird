...
// durable store state in a single read transaction.
func (store *Store) ResolveAuthentication(ctx context.Context, lookup application.AuthenticationLookup) (application.AuthenticationState, error) {
	...
	if err != nil {
		return application.AuthenticationState{}, fmt.Errorf("begin SQLite authentication resolution: %w", err)
	}
	defer func() { _ = tx.Rollback() }()
	return resolveAuthenticationState(ctx, tx, lookup)
}

func resolveAuthenticationState(ctx context.Context, tx *sql.Tx, lookup application.AuthenticationLookup) (application.AuthenticationState, error) {
	...
	if err := tx.QueryRowContext(ctx, "SELECT CAST(unixepoch('subsec') * 1000000 AS INTEGER)").Scan(&serverMicros); err != nil {
		return application.AuthenticationState{}, fmt.Errorf("read SQLite authentication server time: %w", err)
	}
	...
	if err != nil {
		...
		return application.AuthenticationState{}, fmt.Errorf("read SQLite authentication principal: %w", err)
	}
	...
	if deviceID, hasDevice := lookup.DeviceID(); hasDevice {
		...
		if loadErr != nil {
			...
			return application.AuthenticationState{}, fmt.Errorf("read SQLite authentication device: %w", loadErr)
		}
	...
	if sessionID, hasSession := lookup.ActorSessionID(); hasSession {
		...
		if loadErr != nil {
			...
			return application.AuthenticationState{}, fmt.Errorf("read SQLite authentication actor session: %w", loadErr)
		}
...
func verifyGrantCurrency(ctx context.Context, tx *sql.Tx, bound domain.AggregateRef, verifiedAt time.Time) error {
	var status string
	var current uint64
	var expires sql.NullInt64
	err := tx.QueryRowContext(ctx, `SELECT status, version, expires_at_us FROM grants WHERE grant_id = ?`,
	...
	if err != nil {
		return fmt.Errorf("read SQLite authentication grant revision: %w", err)
	}
	if status != "active" || current != bound.Version().Uint64() ||
		expires.Valid && expires.Int64 <= timeMicros(verifiedAt) {
		return queryError(domain.ErrorCodeForbidden, "grant authorization is no longer current")
	}
	return nil
}

func loadInstallationAuthority(ctx context.Context, tx *sql.Tx, installation domain.InstallationID,
	epoch domain.AuthorityEpoch) (domain.AuthorityID, error) {
	var authorityText string
	err := tx.QueryRowContext(ctx, `SELECT authority_id FROM authority_streams
		WHERE scope_kind = 'installation' AND scope_id = ? AND authority_epoch = ?`,
	...
	if err != nil {
		return domain.AuthorityID{}, fmt.Errorf("read SQLite authentication installation authority: %w", err)
	}
