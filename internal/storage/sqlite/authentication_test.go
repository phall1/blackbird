...
func buildAuthenticationFixture(t *testing.T, store *Store, security securityFixture) authenticationFixture {
	...
	workspaceID, _ := domain.ParseWorkspaceID(commandTestUUID(400))
	ownerMembershipID, _ := domain.ParseMembershipID(commandTestUUID(401))
	...
	pairedDeviceID, _ := domain.ParseDeviceID(commandTestUUID(402))
	pairingCeremonyID, _ := domain.ParseCeremonyID(commandTestUUID(403))
	...
	membershipID, _ := domain.ParseMembershipID(commandTestUUID(404))
	membershipCeremonyID, _ := domain.ParseCeremonyID(commandTestUUID(405))
	...
	actorID, _ := domain.ParseActorID(commandTestUUID(406))
	...
	delegationID, _ := domain.ParseActorDelegationID(commandTestUUID(407))
	delegationCeremonyID, _ := domain.ParseCeremonyID(commandTestUUID(408))
	...
	sessionCeremonyID, _ := domain.ParseCeremonyID(commandTestUUID(409))
	...
	sessionID, _ := domain.ParseActorSessionID(commandTestUUID(410))
	sessionClientID, _ := domain.ParseClientInstanceID(commandTestUUID(411))
...
func TestResolveAuthenticationRejectsMismatchedOrStaleIdentity(t *testing.T) {
	...
	rejectedSession(t, "session identity missing", sessionLookup(func(params *application.AuthenticationLookupParams) {
		unknown, _ := domain.ParseActorSessionID(commandTestUUID(999))
	...
	rejectedDevice(t, "device identity missing", deviceLookup(func(params *application.AuthenticationLookupParams) {
		unknown, _ := domain.ParseDeviceID(commandTestUUID(998))
		params.DeviceID = &unknown
	}), domain.ErrorCodeUnauthenticated)

	unknownPrincipal, _ := domain.ParsePrincipalID(commandTestUUID(997))
	...
	t.Run("device revoked", func(t *testing.T) {
		if _, err := store.db.Exec(`UPDATE device_registrations SET status = 'suspended' WHERE device_id = ?`, fixture.ownerDevice.ID().String()); err != nil {
			t.Fatal(err)
		}
		...
		if !errors.As(err, &rejection) || rejection.Code() != domain.ErrorCodeForbidden {
			t.Fatalf("revoked device error=%v, want CommandError %s", err, domain.ErrorCodeForbidden)
		}
	})
}
...
func TestVerifyGrantCurrencyRejectsMissingRevokedStaleOrExpired(t *testing.T) {
	...
	verify := func(t *testing.T, wantRejected bool) {
		t.Helper()
		tx, beginErr := store.db.BeginTx(context.Background(), &sql.TxOptions{ReadOnly: true})
		if beginErr != nil {
			t.Fatal(beginErr)
		}
	...
	t.Run("stale", func(t *testing.T) {
		if _, err := store.db.Exec(`UPDATE grants SET version = version + 1 WHERE grant_id = ?`, grant.ID().String()); err != nil {
			t.Fatal(err)
		}
		verify(t, true)
	})
	t.Run("revoked", func(t *testing.T) {
		if _, err := store.db.Exec(`UPDATE grants SET status = 'revoked' WHERE grant_id = ?`, grant.ID().String()); err != nil {
			t.Fatal(err)
		}
		verify(t, true)
	})
	t.Run("expired", func(t *testing.T) {
		if _, err := store.db.Exec(`UPDATE grants SET expires_at_us = 1 WHERE grant_id = ?`, grant.ID().String()); err != nil {
			t.Fatal(err)
		}
		verify(t, true)
	})
	t.Run("missing", func(t *testing.T) {
		missing, parseErr := domain.ParseGrantID(commandTestUUID(996))
		if parseErr != nil {
			t.Fatal(parseErr)
		}
